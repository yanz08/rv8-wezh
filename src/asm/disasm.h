...
namespace riscv
{
	...
	bool decode_pairs(T &dec, addr_t &addr, std::deque<T> &dec_hist, addr_t pc_bias)
	{
		...
		while (rvxi->addr != rva_none)
		{
			if (rvxi->op2 == dec.op)
			{
				...
				{
					if (rvxi->op1 != li->op && dec.rs1 == li->rd)
						break; // break: another primitive encountered
					if (rvxi->op1 != li->op || dec.rs1 != li->rd)
						continue; // continue: not the right pair
					switch (rvxi->addr)
					{
					case rva_abs:
						addr = li->imm + dec.imm;
	...
	template <typename T> //wezh
	std::string disasm_inst_simpler(T &dec)
	{
		std::string args1;
		args1 += rv_inst_name_sym[dec.op];
		return args1;
	}
	...
	std::string disasm_inst_simple(T &dec)
	{
		...
		while (*fmt)
		{
			...
			case ')':
				...
				break;
			case '0':
				args += rv_ireg_name_sym[dec.rd];
				break;
			case '1':
				args += rv_ireg_name_sym[dec.rs1];
				break;
			case '2':
				args += rv_ireg_name_sym[dec.rs2];
				break;
			case '3':
				args += rv_freg_name_sym[dec.rd];
				break;
			case '4':
				args += rv_freg_name_sym[dec.rs1];
				break;
			case '5':
				args += rv_freg_name_sym[dec.rs2];
				break;
			case '6':
				args += rv_freg_name_sym[dec.rs3];
				break;
			case '7':
				args += format_string("%d", dec.rs1);
				break;
			case 'i':
				args += format_string("%d", dec.imm);
				break;
			case 'o':
				args += format_string("pc %c %td",
									  intptr_t(dec.imm) < 0 ? '-' : '+',
			...
			case 'c':
			{
				...
				if (csr_name)
					args += format_string("%s", csr_name);
				else
					args += format_string("0x%03x", dec.imm & 0xfff);
				break;
			}
			case 'r':
				switch (dec.rm)
